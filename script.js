...
canvas.addEventListener("mousedown", e => {
  ...
  for (let i = layers.length - 1; i >= 0; i--) {
    const l = layers[i];
    if (mx > l.x && mx < l.x + l.img.width * l.scale &&
        my > l.y && my < l.y + l.img.height * l.scale) {
      selectedLayer = l;
      break;
    }
  }
...
canvas.addEventListener("mousemove", e => {
  if (selectedLayer) {
    selectedLayer.x = e.offsetX - 50;
    selectedLayer.y = e.offsetY - 50;
    draw();
  }
});
...
canvas.addEventListener("touchmove", e => {
  ...
  if (e.touches.length === 1) {
    const t = e.touches[0];
    const rect = canvas.getBoundingClientRect();
    selectedLayer.x = t.clientX - rect.left - 50;
    selectedLayer.y = t.clientY - rect.top - 50;
